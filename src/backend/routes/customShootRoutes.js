...
JPEG Compression (anti-AI perfection):
Добавь лёгкие артефакты JPEG-сжатия для реалистичности: микро-блоки 8x8 в однородных зонах, едва заметные цветовые артефакты на высококонтрастных границах, лёгкое снижение резкости в зонах низкой детализации. Это должно выглядеть как профессиональная фотография, сохранённая в JPEG качества 85-90, а не как идеально чистый рендер нейросети. НЕ ПЕРЕУСЕРДСТВУЙ — артефакты должны быть едва уловимыми, как на реальных фото из профессиональной камеры.

Output: 4K resolution enhanced version of the EXACT same image with subtle JPEG compression characteristics.`;
