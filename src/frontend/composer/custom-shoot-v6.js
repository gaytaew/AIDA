/**
 * Custom Shoot V6 Composer (AI-Режиссёр)
 * 
 * Uses Style Presets from V6 AI Director for generation.
 * No manual parameter tuning - style is applied from preset.
...
const state = {
  ...
  // Universe params (Custom Shoot 5 - V5 Smart System) - NOT USED in V6
  universeParams: null,
  universeBlocks: [],
  universeDefaults: {},
  universeValues: {},
  ...
  // V6 Style Presets (AI Director)
  stylePresets: [],           // Available presets from API
...
// V6: Load style presets from AI Director
async function loadStylePresets() {
  try {
    console.log('[LoadStylePresets] Loading from /api/styles...');
    ...
    console.error('[LoadStylePresets] Error:', e.message);
    state.stylePresets = [];
  }
}
...
function renderStylePresetsGrid() {
  ...
  const detailsPanel = document.getElementById('selected-preset-details');

  if (!container) {
    console.warn('[RenderStylePresets] Container not found');
    return;
  }
  ...
  if (state.stylePresets.length === 0) {
    container.innerHTML = `
      <div class="no-presets-message">
        ...
        <div style="margin-bottom: 8px;">Нет сохранённых стилей</div>
        <div><a href="/editors/style-editor.html">Создайте стиль</a> в AI-Режиссёре</div>
      </div>
    `;
    return;
  }

  container.innerHTML = state.stylePresets.map(preset => {
    const isSelected = state.selectedPresetId === preset.id;
    const previewImg = preset.referenceImageUrl
      ? `<img src="${preset.referenceImageUrl}" alt="${escapeHtml(preset.name)}">`
      : '<div style="display: flex; align-items: center; justify-content: center; height: 100%; font-size: 32px;">🎨</div>';

    return `
      <div class="style-preset-card ${isSelected ? 'selected' : ''}" data-preset-id="${preset.id}">
        <div class="preset-preview">${previewImg}</div>
        <div class="preset-name">${escapeHtml(preset.name)}</div>
        <div class="preset-version">v${preset.version || 1}</div>
      </div>
    `;
  }).join('');
  ...
  container.querySelectorAll('.style-preset-card').forEach(card => {
    card.addEventListener('click', () => {
      selectStylePreset(card.dataset.presetId);
    });
  });
...
function selectStylePreset(presetId) {
  ...
  state.selectedPreset = state.stylePresets.find(p => p.id === presetId) || null;
  ...
  // Save to generation settings
  saveGenerationSettings();
}

// V6: Update selected preset details panel
function updateSelectedPresetDetails() {
  const panel = document.getElementById('selected-preset-details');
  const nameEl = document.getElementById('selected-preset-name');
  const promptEl = document.getElementById('selected-preset-prompt');

  if (!panel || !nameEl || !promptEl) return;

  if (state.selectedPreset) {
    nameEl.textContent = state.selectedPreset.name;
    promptEl.textContent = state.selectedPreset.naturalPrompt || 'Нет описания';
    panel.classList.add('active');
  } else {
    panel.classList.remove('active');
  }
...
function renderGeneratePage() {
  // V6: Render style presets grid instead of universe params
  renderStylePresetsGrid();
...
async function generateFrame(frameId) {
  ...
  // V6: Use style preset instead of universe params
  if (!state.selectedPreset) {
    log('ERROR: No style preset selected');
    alert('Сначала выберите стиль (пресет)');
    return;
  }

  log('Using V6 Style Preset', { presetId: state.selectedPreset.id, name: state.selectedPreset.name });

  const params = {
    frameId,
    locationId: elements.genLocation.value || null,
    extraPrompt: elements.genExtraPrompt.value.trim(),
    // V6: Send style params from preset
    styleParams: {
      presetId: state.selectedPreset.id,
      naturalPrompt: state.selectedPreset.naturalPrompt || '',
      antiAiDirectives: state.selectedPreset.antiAiDirectives || [],
      technicalParams: state.selectedPreset.technicalParams || {}
    },
    aspectRatio: elements.genAspectRatio?.value || '3:4',
    imageSize: elements.genImageSize?.value || '2K',
...
async function init() {
  ...
  try {
    ...
    await Promise.all([
      ...
      loadLocations(),
      loadEmotions(),
      loadStylePresets()  // V6: Load style presets instead of universe params
    ]);
